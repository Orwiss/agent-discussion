...
## STEP 6. 오디오 플러그인 설치 (MHAudioPlayer)

> 오디오와 블렌드셰이프를 UE5 안에서 동시에 재생하여 완벽한 립싱크를 구현합니다.

### 6-1. 플러그인 복사

```
agent-discussion 레포의 Plugins/MHAudioPlayer 폴더 전체를
UE5 프로젝트의 Plugins/ 폴더에 복사

예: AgentMH/Plugins/MHAudioPlayer/
```

### 6-2. 프로젝트 빌드

UE5 에디터를 닫고 다시 열면 자동 컴파일됩니다.
에러가 나면 Visual Studio에서 솔루션 빌드.

### 6-3. 플러그인 활성화 확인

```
Edit → Plugins → "MHAudioPlayer" 검색 → 체크 ✅ (보통 자동 활성화)
```

### 6-4. MetaHuman에 컴포넌트 추가

각 MetaHuman Blueprint (BP_MH_UXResearcher, BP_MH_VisualDesigner, BP_MH_SoftwareEngineer)에:

```
Components 패널 → + Add → "MH Audio Player Component" 검색 → 추가
```

BP_MH_BlendshapePlayer 옆에 MHAudioPlayerComponent가 보이면 성공.

---

## STEP 7. BP_OSCManager 수정 (오디오 + 싱크)

### 7-1. 변수 추가

BP_OSCManager를 열고 My Blueprint → Variables → + 추가:

| 이름 | 타입 | 용도 |
|------|------|------|
| BSReadyCharID | String | 블렌드셰이프 준비된 캐릭터 ID |
| AudioReadyCharID | String | 오디오 준비된 캐릭터 ID |

### 7-2. OSC 주소 3개 추가

기존 Switch on String 노드 (OnOscMessage)에 핀 3개 추가:
- `/mh/audio_start`
- `/mh/audio_chunk`
- `/mh/audio_end`

### 7-3. /mh/audio_start 핸들러

```
/mh/audio_start 핀 →

① OSC Message Get String at Index 0 → CharID (String)
② OSC Message Get Int at Index 1 → NumChunks (Integer)
③ Switch on String (CharID)
   → "MH_UXResearcher": Get MH_UXResearcher → Get Component by Class (MHAudioPlayerComponent) → Audio Start (CharID, NumChunks)
   → "MH_VisualDesigner": 동일 패턴
   → "MH_SoftwareEngineer": 동일 패턴
```

### 7-4. /mh/audio_chunk 핸들러

```
/mh/audio_chunk 핀 →

① OSC Message Get String at Index 0 → CharID
② OSC Message Get Int at Index 1 → ChunkIndex
③ OSC Message Get String at Index 2 → Base64Data
④ Switch on String (CharID)
   → 각 캐릭터: Get Component by Class (MHAudioPlayerComponent) → Audio Chunk (CharID, ChunkIndex, Base64Data)
```

### 7-5. /mh/audio_end 핸들러
...
① OSC Message Get String at Index 0 → CharID
② Switch on String (CharID)
   → 각 캐릭터: Get Component by Class (MHAudioPlayerComponent) → Audio End (CharID)
③ Set AudioReadyCharID = CharID
④ Call TryStartPlayback (CharID)
```

### 7-6. /mh/bs_end 핸들러 수정

기존 `/mh/bs_end` 핸들러의 **마지막에** 추가:

```
(기존 EnqueueBlendshapes 호출 유지)
→ Set BSReadyCharID = CharID
→ Call TryStartPlayback (CharID)
```

**중요:** BP_MH_BlendshapePlayer의 EnqueueBlendshapes에서 **자동 시작을 제거**해야 합니다.
- EnqueueBlendshapes 함수 안에서 `BSPlaying = false`일 때 `StartBlendshapes`를 직접 호출하는 부분을 **삭제**
- 데이터 저장만 하고, 시작은 TryStartPlayback에서 담당

### 7-7. TryStartPlayback 함수 만들기

```
My Blueprint → Functions → + 추가 → 이름: TryStartPlayback
Inputs: CharID (String)
```

로직:

```
① Branch: BSReadyCharID == CharID AND AudioReadyCharID == CharID

True →
  ② Switch on String (CharID)
     → 각 캐릭터:
        Get Component by Class (BP_MH_BlendshapePlayer) → Start Blendshapes
        Get Component by Class (MHAudioPlayerComponent) → Play Audio
  ③ Set BSReadyCharID = "" (빈 문자열)
  ④ Set AudioReadyCharID = "" (빈 문자열)

False →
  (아무것도 안 함 — 나머지 하나가 올 때까지 대기)
```

이렇게 하면 블렌드셰이프와 오디오가 **같은 게임 스레드 틱**에서 동시에 시작됩니다.
...
python test_osc.py
```
3. 키 입력 → MetaHuman의 **입 모양 + 소리**가 동시에 나오면 **성공!**
4. 소리가 MetaHuman 위치에서 나오는지 확인 (공간 오디오)
